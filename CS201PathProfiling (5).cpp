...
int edge_x = (atoi(lb_name.c_str()+1)*new_counter);
...
//outs()<<"Malloc1";
edge = (int *)calloc(new_counter*new_counter, sizeof(int));
//outs()<<"Malloc2";
...
//last_block = &BB1; 
//ter_edge++;
for(int q = 0; q<new_counter; q++)
for(int a = 0; a<new_counter; a++) 
if(edge[q*(new_counter)+a]!=0)
outs()<<"\nPath B"<<q<<"->B"<<a<<" "<<edge[q*(new_counter) + a];
